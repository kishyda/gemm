...
__global__ void gemm_05(size_t m, size_t n, size_t k, T alpha, T const* A,
...
template <typename T>
void launch_gemm_05(size_t m, size_t n, size_t k, T const alpha,
                            ...
                            )
{
    ...
    dim3 const grid_dim{
        ...
        (static_cast<unsigned int>(m) + BLOCK_TILE_SIZE_Y - 1U) /
            BLOCK_TILE_SIZE_Y,
        1U};
    gemm_05<T, BLOCK_TILE_SIZE_X, BLOCK_TILE_SIZE_Y, BLOCK_TILE_SIZE_K,
             THREAD_TILE_SIZE_X, THREAD_TILE_SIZE_Y>
...
NB_MODULE(gemm_05, m) {
    m.def("launch_gemm_float", [](size_t m, size_t n, size_t k, float alpha, nb::ndarray<>& A, size_t lda, nb::ndarray<>& B, size_t ldb, float beta, nb::ndarray<>& C, size_t ldc){
        launch_gemm_05<float>(m, n, k, alpha, (float*) A.data(), lda, (float*) B.data(), ldb, beta, (float*) C.data(), ldc);
